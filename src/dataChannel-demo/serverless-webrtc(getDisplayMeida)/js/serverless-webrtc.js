...
var getPositionText = document.getElementsByClassName("positionInformation")[0]
...
function sendMessage () {
  if ($('#messageTextBox').val()) {
    channel = new RTCMultiSession()
    writeToChatLog($('#messageTextBox').val(), 'text-success', true)
      ...
      if (e.data.size) {
        ...
        if (data.type === 'file') {
          fileReceiver1.receive(e.data, {})
              ...
              writeToChatLog(data.message, 'text-info', false)
              // Scroll chat text area to the bottom on new input.
              $('#chatlog').scrollTop($('#chatlog')[0].scrollHeight)
          }else if(data.position){
              console.warn("data.position:",data.position)
              // let span = document.createElement("span")
              let {x:offsetX, y: offsetY} = data.position
              //显示
              var span=document.getElementById('secspan');
              span.innerText="X:"+offsetX+","+"Y:"+offsetY;
              span.style.fontSize='12px';
              span.style.paddingLeft='6px';
              //移动框
              var fir=document.getElementById('secdiv');
              fir.style.display = 'block';
              fir.style.top = offsetY + 'px';
              fir.style.left = offsetX + 'px';
          }
      }
    }
...
function getMousePos(event) {
    ...
    let {x:leftX, y:topY} = localVideo.getBoundingClientRect()
    let offsetX = clientX - leftX
    let offsetY = clientY - topY

    //显示
    var span=document.getElementById('firspan');
    span.innerText="X:"+offsetX+","+"Y:"+offsetY;
    span.style.fontSize='12px';
    span.style.paddingLeft='6px';
    //移动框
    var fir=document.getElementById('firdiv');
    fir.style.display = 'block';
    fir.style.top = offsetY + 'px';
    fir.style.left = offsetX + 'px';

    getPositionText.innerHTML = `offsetX: ${offsetX} offsetY: ${offsetY}`

    if(channel){
        channel.send({position: {x: offsetX, y:offsetY} })
    }

}

localVideo.onmousemove = getMousePos;